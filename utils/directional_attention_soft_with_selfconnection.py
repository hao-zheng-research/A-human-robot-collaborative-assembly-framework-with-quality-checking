...
def calculate_attention_scores(input_tensor, num_prev_frames=5):
    ...
    attention_map_left_hand = torch.cat((attention_map_left_hand[:, :50], attention_map_left_hand[:, 51:]), dim=1)
    attention_map_right_hand = torch.cat((attention_map_right_hand[:, :49], attention_map_right_hand[:, 50:]), dim=1)
